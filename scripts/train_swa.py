...
class TrainState(train_state.TrainState):
    ...
    swa_num_samples: Any = None
    swa_batch_stats: Any = None
    swa_mean_params: Any = None
...
def update_swa_batch_stats(state, batch):
    _, new_model_state = state.apply_fn(
        {
            'params': state.swa_mean_params,
            'batch_stats': state.swa_batch_stats,
            'image_stats': state.image_stats,
        }, batch['images'], mutable=['batch_stats',],
        use_running_average=False,
    )
...
def step_val(params, batch_stats, image_stats, batch, num_classes):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--swa_num_pt_epochs', default=150, required=True, type=int)
    parser.add_argument('--swa_learning_rate', default=0.05, required=True, type=float)
    ...
    initial_batch_stats = copy.deepcopy(var_dict['batch_stats'])
    ...
    scheduler = optax.join_schedules(
        schedules=[
            ...
            optax.constant_schedule(
                value            = args.swa_learning_rate,
            ),
        ], boundaries=[
            args.num_warmup_epochs * trn_steps_per_epoch,
            args.swa_num_pt_epochs * trn_steps_per_epoch,
        ]
    )
    ...
    state = TrainState.create(
        ...
        swa_num_samples = 0,
        swa_batch_stats = var_dict['batch_stats'] if 'batch_stats' in var_dict else {},
        swa_mean_params = jax.tree_util.tree_map(lambda e: jnp.zeros_like(e), var_dict['params']),
    )
    ...
    initial_batch_stats = jax_utils.replicate(initial_batch_stats)
    for epoch_idx in range(epoch_offset + 1, args.num_epochs + 1):
        ...
        if epoch_idx > args.swa_num_pt_epochs:
            ...
            if state.batch_stats:
                state = state.replace(swa_batch_stats=initial_batch_stats)
                trn_loader = dataloaders['dataloader'](rng=data_rng)
                trn_loader = jax_utils.prefetch_to_device(trn_loader, size=2)
                for batch_idx, batch in enumerate(trn_loader, start=1):
                    state = update_swa_batch_stats(state, batch)
                state = state.replace(swa_batch_stats=sync_batch_stats(state.swa_batch_stats))
        ...
        if (epoch_idx == 1
            ...
            or epoch_idx >= args.swa_num_pt_epochs - 1):
            ...
            for batch_idx, batch in enumerate(val_loader, start=1):
                if epoch_idx > args.swa_num_pt_epochs:
                    metrics = step_val(
                        state.swa_mean_params,
                        state.swa_batch_stats,
                        state.image_stats, batch)
                else:
                    metrics = step_val(
                        state.params,
                        state.batch_stats,
